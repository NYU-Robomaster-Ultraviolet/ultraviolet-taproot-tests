[![pipeline status](https://gitlab.com/aruw/controls/taproot-ultraviolet-taproot/badges/develop/pipeline.svg)](https://gitlab.com/aruw/controls/taproot-ultraviolet-taproot/-/commits/develop)

<!-- Start sections that may be removed after forking this repository -->
<hr/>

# taproot-ultraviolet-taproot
...
- Code generated from the `taproot` and `modm` repositories. This generated code is located in
  `/ultraviolet-taproot/taproot` and includes the hardware abstraction layer provided by
...
git remote add template https://gitlab.com/aruw/controls/taproot-ultraviolet-taproot.git
...
build tools. The provided `main.cpp` (`/ultraviolet-taproot/src/main.cpp`) includes initialization of
all core systems and drivers. This should allow you to develop control systems starting day one.

_Note: [Issue #3](https://gitlab.com/aruw/controls/taproot-ultraviolet-taproot/-/issues/3) tracks the
...
swapping your own project name in place of `ultraviolet-taproot`. Furthermore, rename the directory
`/ultraviolet-taproot` accordingly. Feel free to call it the same name as your repo.
...
To start developing software, place your source code in `/ultraviolet-taproot/src` and your tests in
`/ultraviolet-taproot/test`. See the [workflow guide](#workflow-guide) for how to build, test, and
deploy your code.
...
`ultraviolet-taproot` with the new name if it has been renamed):

1. Commit any work so you don't lose it
1. Edit `ultraviolet-taproot/project.xml` according to the included comment
1. Run `rm -r ultraviolet-taproot/taproot`
1. Run `cd ultraviolet-taproot` and then `lbuild build`
...
taproot-ultraviolet-taproot is covered under the GPL-3.0-or-later with the following exceptions:
- `/taproot/modm` and `/ultraviolet-taproot/taproot/modm` are licensed under MPL 2.0 by the modm
  project. We _are not_ the license holder for these files. See `/modm/LICENSE` for license
  information.
- `/ultraviolet-taproot/taproot/src/taproot/algorithms/MahonyAHRS.h` and
  `/ultraviolet-taproot/taproot/src/taproot/algorithms/MahonyAHRS.cpp` are licensed under the GPL by
...
1. Make sure you have VSCode opened in the folder `taproot-ultraviolet-taproot` (**not
   `ultraviolet-taproot`**)
...
1. Open the folder `aruw-ultraviolet-taproot` in VSCode. Hit the debug tab on the left side or type
...
`/ultraviolet-taproot/robot-type/robot_type.hpp` and change the macro defined in this file.
...
The below commands require that your working directory is `/ultraviolet-taproot` (where the
